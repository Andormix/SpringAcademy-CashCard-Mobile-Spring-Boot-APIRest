...
class AndormixCashCardAppProjectApplicationTests {
    ...
    @Test
    @DirtiesContext
    void shouldNotAllowUpdatesToCashCardsTheyDoNotOwn()
    {
        CashCard updateCard = new CashCard(null, 250.00, null);

        ResponseEntity<Void> response = restTemplate
                .withBasicAuth("sarah1", "abc123")
                .exchange("/cashcards/102", HttpMethod.PUT, new HttpEntity<>(updateCard), Void.class);
    ...
    @Test
    @DirtiesContext
    void shouldUpdateAnExistingCashCardTheyOwn()
    {
        // 1. Uso la ID 100L que sí le pertenece a sarah1
        CashCard updateCard = new CashCard(null, 250.00, null);

        ResponseEntity<Void> response = restTemplate
                .withBasicAuth("sarah1", "abc123")
                .exchange("/cashcards/100", HttpMethod.PUT, new HttpEntity<>(updateCard), Void.class);

        // 2. Espero un 204 NO_CONTENT
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

        // Verifica en BD que el cambio realmente ocurrió
        ResponseEntity<String> getResponse = restTemplate
                .withBasicAuth("sarah1", "abc123")
                .getForEntity("/cashcards/100", String.class);

        assertThat(getResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(getResponse.getBody()).contains("250.0");
    ...
    /*@Test
    void shouldCreateAndReturnANewCashCard()
    {

        CashCard newCashCard = new CashCard(null, 1050.00);
        ...
        DocumentContext documentContext = JsonPath.parse(response.getBody());
        Double amount = documentContext.read("$.amount");
        assertThat(amount).isEqualTo(1050.00);
